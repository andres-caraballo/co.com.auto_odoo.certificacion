...
public class ValidacionBonificaciónSalarial implements Question {

    public final CreateDatosNomina crearDatosPersonales;

    public ValidacionBonificaciónSalarial(CreateDatosNomina crearDatosPersonales) {
        this.crearDatosPersonales = crearDatosPersonales;
    }

    public static ValidacionBonificaciónSalarial deNomina(DatosNominaBuilder createDatosPersonales){
        return new ValidacionBonificaciónSalarial(createDatosPersonales.build());
    }
    ...
    public Object answeredBy(Actor actor) {
        ...
        String parqueaderoEnPantalla = Text.of(PARQUEADERO).viewedBy(actor).asString();
        String auxilioNoPrestacionalConectividadEnPantalla = Text.of(AUXILIO_NO_PRESTACIONAL_CONECTIVIDAD).viewedBy(actor).asString();
        String comisionesEnPantalla = Text.of(COMISIONES).viewedBy(actor).asString();
        ...
        String parqueadero = crearDatosPersonales.getParqueadero();
        String auxilioNoPrestacionalConectividad = crearDatosPersonales.getConectividad();
        String comisiones = crearDatosPersonales.getComisiones();
        ...
        Ensure.that(auxilioNoPrestacionalAlimentacionEnPantalla).isEqualTo(auxilioNoPrestacionalAlimentacion)),
                Task.where("Valida Resultado de parqueadero",
                        Ensure.that(parqueaderoEnPantalla).isEqualTo(parqueadero)),
        ...
        Ensure.that(auxilioNoPrestacionalConectividadEnPantalla).isEqualTo(auxilioNoPrestacionalConectividad)),
                Task.where("Valida Resultado de Comisiones",
                        Ensure.that(comisionesEnPantalla).isEqualTo(comisiones)),
